...
<body>
	...
	<div class="container" id="main">
		...
		<div id="article">
			...
			<div>
				<div class="f_div">
					...
					<c:choose>
						<c:when test="${article_pre!=null}">
							<a href="/blog/ArticleServlet?id=${article_pre.id}">&nbsp;다음글:${article_pre.title}</a>
						</c:when>					
						<c:otherwise>
							&nbsp;没有更早的文章了
						</c:otherwise>					
				...
				<div class="r_div">
						...
						<c:when test="${article_next!=null}">
							<a href="/blog/ArticleServlet?id=${article_next.id}">이전글:&nbsp;${article_next.title}</a>
						</c:when>					
						<c:otherwise>
							&nbsp;没有更多的文章了
						</c:otherwise>					
				...
				<div>			
				<span class="btn btn-default" style="color:#d9534f;"  onclick="love_article(${article.id})" >관심</span>
				</div>						
				<br/>
			</div>
			...
			<div class="r_div">
			<a href="#comment"><span class="glyphicon glyphicon-pencil">&nbsp;코멘트</span></a>
			</div>
			...
			<input  style="width:30%" class="form-control" type="text" name="w_nickname" value="anonymous"  >
			<br/>							
